-- Games can now carry an edition (Deluxe, Collector's, ...) like special
-- editions and steelbooks do, stored as free text in `edition_name`.
--
-- `all_items` gains an `edition_name` column so listing cards can render the
-- edition badge for every type that has one. `subtitle` is unchanged (still
-- the developer for games, the edition name for special editions/steelbooks).

alter table public.games add column edition_name text;
