-- `game_genres` had no ordering column, so "primary genre" (used by the card
-- icon and, incidentally, the detail page's chip order) was really just
-- Postgres's unspecified physical row order — not something the app actually
-- controlled. Add an explicit `position` so the first genre picked in the
-- form (or returned by RAWG autofill, in RAWG's own order) is reliably
-- "genre #0" everywhere.

alter table public.game_genres add column position smallint not null default 0;

-- Backfill existing rows with a deterministic order (alphabetical by genre
-- name) since their original selection/RAWG order was never recorded.
with ranked as (
  select
    gg.game_id,
    gg.genre_id,
    row_number() over (partition by gg.game_id order by gn.name) - 1 as rn
...
from ranked
where gg.game_id = ranked.game_id and gg.genre_id = ranked.genre_id;
