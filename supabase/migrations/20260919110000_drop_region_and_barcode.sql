-- Region and barcode are no longer tracked. Drop both columns from the item
-- tables that had them (games: region + barcode; special_editions/steelbooks:
-- region) and rebuild `all_items` without its `region` column.
--
-- WARNING: this permanently deletes the stored region/barcode values.
--
-- The view is rebuilt from the latest definition (20260919100000), with the
-- primary-genre subqueries ordered by `game_genres.position` (as introduced in
-- 20260918110000) — the edition migration had reverted that to `order by name`.
...
alter table public.games drop column region, drop column barcode;
alter table public.special_editions drop column region;
alter table public.steelbooks drop column region;
