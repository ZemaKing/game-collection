-- Add the primary genre (first by name) to each `all_items` row, so listing
-- cards can show a genre icon without an extra query per card. Only games
-- have genres today; every other item type gets nulls.
